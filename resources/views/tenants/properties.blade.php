        ...
        <div class="property-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow" 
            ...
            <div class="p-6">
                ...
                <div class="flex space-x-2">
                    ...
                    @if(($property->units_count - $property->occupied_units) > 0)
                    <button onclick="rentProperty({{ $property->prop_id }})" class="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded-lg font-medium transition-colors flex items-center justify-center space-x-2 rent-btn" data-property-id="{{ $property->prop_id }}">
                        <i class="fas fa-home text-sm"></i>
                        <span>Rent Now</span>
                    </button>
                    @else
                    <button disabled class="flex-1 bg-gray-400 text-white py-2 px-4 rounded-lg font-medium flex items-center justify-center space-x-2 cursor-not-allowed">
...
@push('modals')
    ...
    <div id="rentConfirmModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
            <div class="flex items-center justify-between p-6 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-900">Confirm Rental</h2>
                <button type="button" onclick="closeRentConfirmModal()" class="text-gray-400 hover:text-gray-600">
            ...
            <div class="p-6">
                <div class="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                        <h3 id="confirmPropertyName" class="font-semibold text-gray-900"></h3>
                        <p id="confirmPropertyPrice" class="text-sm text-gray-600"></p>
                    </div>
                </div>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                    <div class="flex items-center">
                        ...
                        <p class="text-sm text-yellow-800">
                            Your rental request will be sent to the landlord for approval. You'll be notified once it's approved.
                        </p>
                    </div>
                </div>
                ...
                <div class="space-y-3 text-sm text-gray-600 mb-4">
                    <div class="flex justify-between">
                        ...
                        <span id="confirmRentAmount" class="font-medium text-gray-900"></span>
                    </div>
                    <div class="flex justify-between">
                        <span>Security Deposit:</span>
                        <span id="confirmDepositAmount" class="font-medium text-gray-900"></span>
                    </div>
                    <div class="flex justify-between border-t border-gray-200 pt-2">
                        <span>Total Initial Payment:</span>
                        <span id="confirmTotalAmount" class="font-medium text-green-600"></span>
                    </div>
                </div>
            </div>
            <div class="flex items-center justify-end space-x-3 p-6 border-t border-gray-200">
                <button type="button" onclick="closeRentConfirmModal()" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                    Cancel
                </button>
                <button type="button" id="confirmRentBtn" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg flex items-center space-x-2">
...
function rentProperty(propertyId) {
    ...
    // Show modal
    document.getElementById('rentConfirmModal').classList.remove('hidden');
    document.getElementById('rentConfirmModal').classList.add('flex');
}

function closeRentConfirmModal() {
    document.getElementById('rentConfirmModal').classList.add('hidden');
    document.getElementById('rentConfirmModal').classList.remove('flex');
    currentPropertyId = null;
}

// View modal functions
function openViewModal(propertyId) {
    fetch(`/tenants/properties/${propertyId}`)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('confirmRentBtn').addEventListener('click', function() {
        ...
        // Send rental request
        fetch(`/tenants/properties/${currentPropertyId}/rent`, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                showNotification('Rental request submitted successfully! Waiting for landlord approval.', 'success');
                closeRentConfirmModal();
                
                // Disable the rent button for this property
                const rentBtn = document.querySelector(`[data-property-id="${currentPropertyId}"]`);
                if (rentBtn) {
                    rentBtn.innerHTML = '<i class="fas fa-clock text-sm"></i><span>Pending Approval</span>';
                    rentBtn.classList.remove('bg-green-600', 'hover:bg-green-700');
                    rentBtn.classList.add('bg-yellow-600', 'cursor-not-allowed');
                    rentBtn.disabled = true;
                    rentBtn.onclick = null;
                }
            } else {
                showNotification(data.message || 'Failed to submit rental request', 'error');
            }
        })
        .catch(error => {
